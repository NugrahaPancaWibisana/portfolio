...
export default function Hero() {
  return (
    ...
    >
      <div className="flex w-full max-w-7xl flex-col items-center gap-12 lg:flex-row lg:items-center lg:justify-between lg:gap-20">
        <div className="flex w-full flex-col gap-6 lg:w-2/3">
          <div className="space-y-3">
            <h2 className="text-2xl font-normal sm:text-3xl sm:font-bold lg:text-4xl">
              {heroData.name}
            </h2>
            <p className="text-lg font-normal text-gray-700 sm:text-2xl lg:text-3xl">
              {heroData.role}
            </p>
            <p className="text-sm leading-relaxed font-normal text-black/60">
          ...
          <div className="flex w-full flex-col items-center gap-4 rounded-lg border-2 p-6 sm:flex-row sm:justify-between">
            <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:gap-4">
              {heroData.buttons.map((button, index) => (
                <a
                  key={index}
                  href={button.link}
                  className={`rounded-lg px-6 py-3 text-center font-medium transition-colors ${
                    button.type === "primary"
            ...
            <div className="flex items-center justify-center gap-4">
              {heroData.socials.map((social, index) => (
                <a
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-full border-2 transition-colors hover:bg-black hover:text-white"
        ...
        >
          ...
          <pre className="overflow-x-auto p-6 text-xs sm:text-sm">
            <code className="text-gray-800">
              ...
              <div className="space-y-1 pl-4">
                ...
                <p>
                  ...
                  <span className="text-green-700">
                    "{heroData.codeSnippet.role}"
                  </span>
                  ,
                </p>
                <p>
                  <span className="text-purple-700">"skills"</span>: [
                  {heroData.codeSnippet.skills.map((skill, index) => (
                    <span key={index}>
                      <span className="text-green-700">"{skill}"</span>
                      {index < heroData.codeSnippet.skills.length - 1 && ", "}
                    </span>
                  ))}
                  ],
                </p>
                ...
                <p>
                  ...
                  <span className="text-blue-700">
                    {heroData.codeSnippet.available.toString()}
                  </span>
                </p>
              </div>
              <p className="text-gray-500">{"}"}</p>
